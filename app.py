...
class Devices(db.Model):
    ...
    def __init__(self, device_id, device_name, config):
...
# POST endpoint that adds the stored config if does not exists
...
def update_device_by_id(device_id):
    ...
    if recent_config and recent_config.config == data['config']:
        return jsonify({'message': 'Device config already exists.'}), 400
    
    # Insert the new config into the database
    _add = Devices(device_id, data['device_name'], data['config'])
    db.session.add(_add)
    db.session.commit()
